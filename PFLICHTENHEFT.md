...
- **Toggle „9-Loch-Runde" / „18-Loch-Runde"** (Default: 9-Loch-Runde, seit 18.09.2026 – zuvor 18-Loch-Runde, siehe Abschnitt 18). Bestimmt, wie viele Lochseiten der Pager (3.2) enthält und ob die rechte Lochliste angezeigt wird. Der Toggle ist **jederzeit während der Runde änderbar**, auch nach bereits erfassten Scores (bewusste Vereinfachung, siehe „Getroffene Annahmen" unter 10).
...
- Zeigt je Lochseite das **echte, bereinigte Foto der Platzausschilderung** dieser Bahn (seit 17.09.2026, siehe Abschnitt 15 – zuvor eine individuelle, handgezeichnet wirkende Skizze, siehe 10.5) sowie einen **Tipp-Button** (Glühbirnen-Symbol, seit 18.09.2026 – zuvor ein (i)-Info-Icon, siehe Abschnitt 18) neben der Lochnummer, der den Charakteristik-Text aus 4a bei Bedarf groß in einem Dialog anzeigt (nicht mehr dauerhaft auf der Seite sichtbar), seit 18.09.2026 auf Deutsch/Englisch/Niederländisch passend zur gewählten App-Sprache (siehe Abschnitt 18).
...
- ~~**Namensfeld auf der Startseite**~~ – **erledigt (18.09.2026):** Eingabefeld für den eigenen Namen auf der Startseite, dauerhaft gespeichert und künftig immer vorbelegt (siehe Abschnitt 18).
- ~~**9-Bahnen-Runde als Standardauswahl**~~ – **erledigt (18.09.2026):** Bei „Neue Runde“ ist jetzt die 9-Bahnen-Runde vorausgewählt statt zuvor die 18-Bahnen-Runde (siehe Abschnitt 18).
- ~~**Datum und Wetterbedingungen auf der Scorecard festhalten**~~ – **erledigt (18.09.2026):** Datum und (bei erfolgreichem automatischem Abruf) die Wetterbedingungen am Platz werden dauerhaft mit der Runde gespeichert (siehe Abschnitt 18).
- ~~**Anderes Symbol statt Info-Icon für den Charakteristik-Text-Button**~~ – zunächst (18.09.2026) durch ein Glühbirnen-Symbol ersetzt (Abschnitt 18), auf Nutzer-Feedback am 19.09.2026 wieder auf das (i)-Info-Icon zurückgesetzt (Abschnitt 20).
- ~~**Vollständige Übersetzung aller App-Texte auf Englisch und Niederländisch**~~ – **erledigt (18.09.2026):** Bahn-Charakteristik-Texte (Titel und Beschreibung) liegen jetzt ebenfalls auf Deutsch, Englisch und Niederländisch vor (siehe Abschnitt 18). Bewusst unverändert bleibt der `scorecard_pdf_service.dart`-PDF-Export, der wie bisher fest auf Deutsch bleibt (siehe Abschnitt 18).
...
| `main_pager_flow_test.dart` | 18-Loch-Runde zeigt zwei gefüllte Reiterleisten, 9-Loch-Runde nur noch die linke (rechte Reiterleiste wird komplett entfernt statt leer mitgerendert, siehe Abschnitt 21); Tippen auf einen Lochreiter springt sofort (ohne Animation) auf die Lochseite; der Scorecard-Reiter landet auf der letzten Pager-Seite. |
...
| `player_service_test.dart` | Name ist standardmäßig leer; `setName` persistiert, benachrichtigt Listener und wird beim erneuten Laden übernommen; Trimmen von Leerzeichen, leerer String entfernt den gespeicherten Namen wieder. |
| `weather_service_test.dart` | WMO-Wettercodes werden korrekt auf die 8 Kategorien gemappt; unbekannte Codes fallen auf „bewölkt“ zurück; jede Kategorie hat Icon, i18n-Key und deutsches PDF-Label. |
...
**Stolperfalle „Future.delayed in `testWidgets()`“ (ergänzt am 18.09.2026):** In einem `testWidgets()`-Test NIEMALS `await Future.delayed(...)` verwenden, um auf einen Hintergrund-Vorgang (z. B. den `unawaited()`-Wetterabruf in `RoundsService.createNewRound()`) zu warten – `Future.delayed` legt intern einen echten `Timer` an, und Flutters `TestWidgetsFlutterBinding` lässt Timer nur durch explizites `tester.pump(...)` weiterlaufen, niemals von selbst. Ohne Pump hängt der Test bis zum CI-Timeout (hier: 10 Minuten). Fix: `await tester.pump();` nach `pumpWidget()` verwenden – das lässt anstehende Microtasks (z. B. eine bereits als `Future` abgeschlossene Fake-Antwort) durchlaufen und baut den `Provider`-Baum neu, ganz ohne echten Timer. In einem einfachen `test()` (ohne `testWidgets`, also ohne FakeAsync-Bindings) funktioniert `Future.delayed(Duration.zero)` dagegen wie gewohnt (siehe `rounds_service_test.dart`).

---

## 17. Layout-Bugfix: asymmetrische Ränder im 9-Bahnen-Modus (behoben am 18.09.2026)

Nutzer meldete: Bei einer 18-Bahnen-Runde ist die Bahnenkarte auf der Lochseite korrekt zentriert, bei einer 9-Bahnen-Runde ist der linke Rand der Karte doppelt so breit wie der rechte.

**Ursache:** In `main_pager_screen.dart` bilden zwei `BookTabRail`-Reiterleisten (links/rechts, siehe Abschnitt 11) zusammen mit dem `Expanded`-Pager-Bereich eine `Row`. Die rechte Reiterleiste (Bahnen 10–18) wurde bei einer 9-Bahnen-Runde bisher komplett aus der `Row` entfernt (`if (rightItems.isNotEmpty) ...`), statt nur leer zu bleiben. Dadurch reservierte im 9-Bahnen-Modus nur die linke Leiste horizontalen Platz (ca. 48 px inkl. Padding) – der mittlere Pager-Bereich war dadurch nicht mehr symmetrisch zwischen zwei gleich breiten Rändern eingebettet, wodurch zentrierte Inhalte (z. B. die Bahnenkarte auf der Lochseite, Abschnitt 15) sichtbar nach rechts verschoben wirkten.

**Fix:** Die rechte Reiterleiste wird jetzt immer gerendert, im 9-Bahnen-Modus jedoch mit einer leeren Reiter-Liste (keine Reiter sichtbar, aber derselbe reservierte Platz wie links). `test/main_pager_flow_test.dart` entsprechend angepasst: prüft jetzt, dass in beiden Modi zwei `BookTabRail`-Widgets im Baum stehen, im 9-Bahnen-Modus aber `items` der rechten Leiste leer ist.

---

## 18. Fünf Nutzerwünsche umgesetzt: Namensfeld, 9-Bahnen-Default, Datum/Wetter, Tipp-Icon, EN/NL-Übersetzung (umgesetzt am 18.09.2026)

Fünf zuvor in Abschnitt 8 vorgemerkte Nutzerwünsche (siehe dortige Einträge, alle vom 18.09.2026) wurden in einem gemeinsamen Durchgang umgesetzt:

**1. Namensfeld auf der Startseite.** Neuer Service `lib/services/player_service.dart` (`PlayerService`, analog zu `LocaleService`: `ChangeNotifier` + `SharedPreferences`, Key `gcwbb_player_name`). `start_screen.dart` (jetzt `StatefulWidget`) zeigt ein `TextField` unterhalb des Untertitels, vorbelegt mit dem gespeicherten Namen; jede Änderung wird sofort über `PlayerService.setName()` persistiert (Trimmen von Leerzeichen, leerer String entfernt den gespeicherten Namen wieder). Der Name wird aktuell ausschließlich auf der Startseite verwendet (keine Verknüpfung mit einzelnen Runden) – das kann bei Bedarf später ergänzt werden.

**2. 9-Bahnen-Runde als neuer Default.** `RoundsService.createNewRound()` legt neue Runden jetzt mit `holeCount: 9` an (zuvor 18); der Fallback-Getter `currentHoleCount` (greift nur, solange keine Runde existiert) wurde ebenfalls von 18 auf 9 geändert. Betroffene Tests (`rounds_service_test.dart`, `main_pager_flow_test.dart`) angepasst – Letzterer ruft jetzt explizit `setCurrentHoleCount(18)` auf, um weiterhin den 18-Bahnen-Fall zu prüfen.

**3. Datum und Wetterbedingungen an der Scorecard.** Das Rundendatum wurde bereits zuvor angezeigt (5e); neu ist der automatische Wetterabruf: Neuer `lib/services/weather_service.dart` ruft beim Anlegen einer Runde (`RoundsService.createNewRound()`, nicht-blockierend im Hintergrund) einmalig die aktuellen Wetterbedingungen von [Open-Meteo](https://open-meteo.com) ab (kostenlos, kein API-Key, CORS-fähig für Web-Apps) und hinterlegt sie unveränderlich an der Runde (`Round.weatherTempC`/`weatherCode`/`weatherWindKph`, persistiert in `toJson`/`fromJson`). Bewusste Design-Entscheidung: statt den **Gerätestandort** abzufragen (Berechtigungsdialog, evtl. abweichend vom Spielort), werden **feste Koordinaten des GC Winterberg** verwendet (51,1925° N / 8,5347° E, Stadtzentrum Winterberg – für eine Wettervorhersage ausreichend genau) – die Wetterbedingungen sollen den Platz beschreiben, nicht das exakte GPS-Signal des Telefons. Schlägt der Abruf fehl (offline, Timeout, o. Ä.), bleibt die Runde einfach ohne Wetterangabe – niemals blockierend. `scorecard_page.dart` zeigt Wettericon + Label + Temperatur neben dem Datum, sobald vorhanden (reaktiv über `Provider`, aktualisiert sich also automatisch, sobald der Hintergrund-Abruf fertig ist); `scorecard_pdf_service.dart` ergänzt eine Wetterzeile im PDF-Export (bewusst weiterhin festes Deutsch, siehe Punkt 5 unten). Ein austauschbarer Top-Level-Hook `weatherFetcher` in `weather_service.dart` sorgt dafür, dass `flutter test` nie einen echten Netzwerkaufruf auslöst (alle betroffenen Tests setzen ihn in `setUp()` auf einen Fake).

**4. Tipp-Icon statt Info-Icon.** Nutzer-Feedback: das bisherige (i)-Info-Icon auf der Bahnseite (Abschnitt 15) passte inhaltlich nicht, da es sich um einen Spieltipp handelt. `hole_page.dart`: `Icons.info_outline` → `Icons.lightbulb_outline`; i18n-Key `holeInfo` entsprechend von „Charakteristik anzeigen" auf „Spieltipp anzeigen" (DE) bzw. „Show playing tip" (EN) / „Speeltip tonen" (NL) umbenannt. Das (i)-Info-Icon im „Über diese App"-Menüpunkt (3.3) ist davon nicht betroffen – dort ist es inhaltlich korrekt.

**5. Vollständige EN/NL-Übersetzung der Bahn-Charakteristik-Texte.** `lib/models/hole.dart`: `title`/`text` sind jetzt `Map<String, String>` (Keys `de`/`en`/`nl`) statt einzelner Strings, mit Hilfsmethoden `titleFor(lang)`/`textFor(lang)` (Fallback-Kette wie `LocaleService.t()`: gewünschte Sprache → Deutsch). `lib/content/course_data.dart` enthält jetzt für alle 9 Bahnen sinngemäße englische und niederländische Übersetzungen von Titel und Charakteristik-Text (Deutsch bleibt die Quelle, golfclub-winterberg.de, siehe 4a). `hole_page.dart` zeigt den Text jetzt über `holeData.textFor(locale.lang)` in der aktuell gewählten App-Sprache. Bewusst **nicht** übersetzt: der PDF-Scorecard-Export (`scorecard_pdf_service.dart`), der schon vor dieser Änderung unabhängig von der App-Sprache fest auf Deutsch war (Spaltenköpfe, „Datum:", „9-Bahnen-Runde" etc.) – das wird hier konsistent fortgeführt, eine vollständige PDF-Lokalisierung war nicht Teil dieses Wunsches und bleibt ein möglicher späterer Ausbaupunkt.

**Neue/angepasste Tests:** `player_service_test.dart` (neu), `weather_service_test.dart` (neu, reine WMO-Code-Zuordnung ohne Netzwerk), `rounds_service_test.dart` (9-Bahnen-Default, zwei neue Fälle für erfolgreichen/fehlgeschlagenen Wetterabruf via Fake-`weatherFetcher`), `scorecard_page_test.dart` (neuer Fall: Wetteranzeige sobald vorhanden), `scorecard_pdf_service_test.dart` (neuer Fall: gültiges PDF mit Wetterdaten), `hole_page_test.dart` (Icon-Selektor aktualisiert, neuer Fall: Charakteristik-Text auf Englisch), `main_pager_flow_test.dart` (explizites `setCurrentHoleCount(18)`, `weatherFetcher`-Fake), `widget_test.dart` (`weatherFetcher`-Fake, neuer Fall: Namensfeld leer beim ersten Start und persistiert Eingaben), `locale_service_test.dart` (neuer Fall: alle drei Sprachen kennen dieselben neuen Keys – Schutz vor Übersetzungs-Drift). Neue Abhängigkeit `http: ^1.2.2` in `pubspec.yaml` für den Wetterabruf.

---

## 19. Bahnenbilder bereinigt: Restliche Original-Texte entfernt, Grüntiefe neu gerendert (umgesetzt am 18.09.2026)

Nutzer meldete: Auf Bahn 5 und 6 seien noch nicht alle Texte der Bahnenbilder (Abschnitt 15) ersetzt worden, u. a. eine Grüntiefe-Angabe und ein Entfernungsmesser-Bereich im Original-Look. Bei der Prüfung stellte sich heraus, dass die bisherige Bereinigung (Abschnitt 15) ausschließlich die beiden großen Infokacheln (BAHN/PAR/HCP sowie HERREN/DAMEN) neu gerendert hatte – kleinere Elemente aus den Original-Fotos der Vereins-Website waren auf praktisch allen 9 Bahnenbildern unangetastet geblieben: kleine grüne Wegweiser-Schilder (Richtung nächster Abschlag o. Ä., Bahn 1, 3, 4, 6, 7, 8, 9), eine „GRÜNTIEFE"-Angabe auf allen drei Par-3-Bahnen (Bahn 3: 16 m, Bahn 5: 21 m, Bahn 8: 25 m) sowie eine rotierte Kleinbeschriftung auf Bahn 9. Per Rückfrage hat der Nutzer den Umfang bestätigt: alle 9 Bahnenbilder bereinigen (nicht nur 5/6), die Grüntiefe-Angabe sauber neu rendern (statt entfernen), übrige Alttexte wegretuschieren.

**Umsetzung:** Mit OpenCV (`cv2.inpaint`, Farbdistanz-Masken je Element plus gezielte Nachbereinigung von Anti-Aliasing-Resten) wurden die betroffenen Bildbereiche entfernt und die Umgebung (Baumkronen, Fairway, Wegmarkierungen) rekonstruiert; auf Bahn 7 zusätzlich ein Textur-Klon aus einer unbeteiligten Stelle desselben Bildes, da dort reine Inpainting-Rekonstruktion einen zu glatten Fleck ergab. Die Grüntiefe-Angabe wurde anschließend mit der App-eigenen Schrift (Manrope, aus `assets/fonts/`) sauber neu gesetzt – gleiche Position, gleicher Wert, scharfe Typografie statt Originaltext, damit sie sich in den Gesamtstil der bereits neu gerenderten Infokacheln einfügt. Die bewusst unverändert belassenen Entfernungsmarkierungen (farbige Punkte mit Zahlen im Fairway, bekannte Einschränkung aus Abschnitt 15/`feature_overview_content.dart`) sind davon nicht betroffen. Betroffene Dateien: `assets/holes/bahn1.png`, `bahn3.png`, `bahn4.png`, `bahn5.png`, `bahn6.png`, `bahn7.png`, `bahn8.png`, `bahn9.png` (Bahn 2 hatte keine Restelemente und blieb unverändert). Keine Code-Änderung nötig, reine Asset-Bereinigung.

## 20. Info-Icon statt Glühbirne, zweite Bereinigungsrunde der Bahnenbilder (umgesetzt am 19.09.2026)

Nutzer-Feedback: Das in Abschnitt 18 eingeführte Glühbirnen-Symbol für den Charakteristik-Text-Button wirkte optisch nicht gut. `hole_page.dart`: `Icons.lightbulb_outline` → `Icons.info_outline` zurückgesetzt; i18n-Key `holeInfo` entsprechend wieder auf den ursprünglichen Wortlaut vor Abschnitt 18 zurückgesetzt („Charakteristik anzeigen" DE / „Show characteristics" EN / „Kenmerken tonen" NL). Funktionsumfang-Text („Tipp-Button (Glühbirnen-Symbol)") entsprechend angepasst. `hole_page_test.dart` auf `Icons.info_outline` umgestellt.

Zusätzlich hat der Nutzer gebeten, alle 9 Bahnenbilder nochmals auf verbliebenen Original-Fototext zu prüfen. Dabei gefunden und bereinigt:
- **Bahn 1:** ein bislang übersehenes „CLUBHAUS"-Wegweiser-Label (unscharfer Originaltext neben einem Gebäude-Icon am Wegrand) – Text war lesbar und wurde, analog zur Grüntiefe-Angabe (Abschnitt 19), mit der App-eigenen Schrift (Manrope, Bold, Tracking) sauber neu gesetzt statt entfernt, da er als Wegweiser-Information einen echten Nutzen hat.
- **Bahn 9:** ein zweites, bislang unentdecktes rotiertes Kleintext-Element neben der Entfernungs-Legende (zusätzlich zu dem bereits in Abschnitt 19 behobenen Element auf derselben Bahn). Der Text war trotz mehrfacher Vergrößerung/Kontrastverstärkung nicht mit Sicherheit zu entziffern; auf Nachfrage hat der Nutzer entschieden, ihn wegzuretuschieren statt zu rekonstruieren.

Alle übrigen 7 Bahnenbilder (2–8, jeweils ohne 9) wurden nochmals einzeln visuell geprüft – keine weiteren Restfunde; verbliebene dunkle Flächen sind durchweg legitime Kartenelemente (Hütten-/Gebäude-Icons, Richtungspfeile, Sprinkler-/Pfahl-Symbole, Mähstreifen-Textur). Betroffene Dateien: `assets/holes/bahn1.png`, `bahn9.png`. Keine weitere Code-Änderung außer dem Icon-Fix oben.

---

## 21. Korrektur zu Abschnitt 17: rechte Reiterleiste im 9-Bahnen-Modus wieder entfernt (behoben am 20.09.2026)

Nutzer meldete erneut per Screenshot (mit handschriftlichen Markierungen): im 9-Bahnen-Modus ist der rechte Rand der Bahnenkarte auf der Lochseite weiterhin deutlich breiter als der linke – diesmal in die entgegengesetzte Richtung wie beim ursprünglichen Bug aus Abschnitt 17.

**Ursache:** Der Abschnitt-17-Fix (rechte Reiterleiste im 9-Bahnen-Modus leer, aber weiterhin mit vollem Platzbedarf mitrendern) balancierte zwar rechnerisch die Distanz der Bahnenkarte zu den jeweils äußersten Bildschirmrändern (links ca. 64 px, rechts ca. 64 px an einem 375 px breiten Gerät) – sichtbar ist davon aber nur, was tatsächlich Inhalt hat: links eine gefüllte, farbige Reiterleiste mit den Lochnummern direkt neben der Karte (nur ca. 16 px Abstand dazwischen), rechts dagegen eine komplett leere, unstrukturierte Fläche über die volle reservierte Breite (ca. 64 px). Der eigentlich identische Randwert wird dadurch vom Auge ganz unterschiedlich wahrgenommen – eine sichtbare, zweckvolle Leiste liest sich nicht als „Rand“, eine leere Fläche schon. Die Kennzahl aus Abschnitt 17 (Abstand zum äußersten Bildschirmrand) war also der falsche Maßstab; maßgeblich ist der sichtbare Abstand rund um die Karte selbst.

**Fix:** Die rechte Reiterleiste wird im 9-Bahnen-Modus jetzt wieder komplett aus der `Row` entfernt (`if (rightItems.isNotEmpty) ...` in `main_pager_screen.dart`, Umkehrung des Abschnitt-17-Fixes). Die Bahnenkarte bemisst sich dadurch an der verbleibenden Breite nach der linken Leiste; ihr eigenes `SingleChildScrollView`-Padding (16 px) sorgt links wie rechts für denselben sichtbaren Abstand. Auf einem simulierten iPhone-Viewport (375 px) im Browser verifiziert: Abstand zwischen Leiste und Karte links sowie zwischen Karte und Bildschirmrand rechts jeweils ca. 16 px.

`test/main_pager_flow_test.dart` entsprechend angepasst: prüft im 9-Bahnen-Modus jetzt, dass nur noch ein einziges `BookTabRail`-Widget (links) im Baum steht, statt zwei mit leerer rechter Liste. Die Testfallbeschreibung in Abschnitt 16 wurde entsprechend korrigiert.
...
- 18.09.2026: Layout-Bug behoben (Abschnitt 17): Im 9-Bahnen-Modus fehlte der rechte Gegenpart zur linken Reiterleiste, wodurch zentrierte Seiteninhalte asymmetrisch nach rechts verschoben wirkten (linker Rand doppelt so breit wie rechter, vom Nutzer per Screenshot-Beobachtung gemeldet). Rechte Reiterleiste wird jetzt immer mitgerendert (im 9-Bahnen-Modus leer statt entfernt), Test angepasst.
- 18.09.2026: Fünf zuvor vorgemerkte Nutzerwünsche umgesetzt (Abschnitt 18): Namensfeld auf der Startseite mit dauerhafter Vorbelegung (neuer `PlayerService`); 9-Bahnen-Runde als neuer Default statt 18-Bahnen-Runde; automatischer, nicht-blockierender Wetterabruf (Open-Meteo, feste Platz-Koordinaten statt Gerätestandort) beim Anlegen einer Runde, Anzeige auf der Scorecard und im PDF-Export; Tipp-Icon (Glühbirne statt (i)-Info-Icon) für den Charakteristik-Text-Button; Bahn-Charakteristik-Texte (Titel + Beschreibung) jetzt auf Deutsch/Englisch/Niederländisch. Neue Tests `player_service_test.dart`, `weather_service_test.dart`; mehrere bestehende Testdateien angepasst (9-Bahnen-Default, Icon-Selektor, Wetter-Fake). Neue Abhängigkeit `http`. Beim ersten CI-Lauf hing danach ein Test 10 Minuten bis zum Timeout (“Scorecard zeigt Wetterbedingungen an, sobald der Abruf vorliegt”); über einen temporären Diagnose-Schritt (JSON-Reporter + Annotations) auf ein `Future.delayed()` innerhalb eines `testWidgets()`-Tests zurückgeführt, das ohne `tester.pump()` in der FakeAsync-Testumgebung nie feuert (Stolperfalle neu in Abschnitt 16 dokumentiert). Danach war die CI grün, Build und Deploy erfolgreich, alle fünf Funktionen live unter https://carstenroesner.github.io/gc-winterberg-birdie-book/ verifiziert.
- 18.09.2026: Bahnenbilder bereinigt (Abschnitt 19): auf allen 9 Bahnenbildern verbliebene Original-Textelemente (kleine Wegweiser-Schilder, Grüntiefe-Angabe auf den drei Par-3-Bahnen, eine rotierte Kleinbeschriftung) per Bildbearbeitung (OpenCV-Inpainting) entfernt bzw. – bei der Grüntiefe – sauber mit der App-eigenen Schrift neu gerendert.
- 19.09.2026: Info-Icon statt Glühbirne zurückgesetzt und zweite Bereinigungsrunde der Bahnenbilder (Abschnitt 20): Nutzer-Feedback, dass das Glühbirnen-Symbol optisch nicht überzeugte, Icons.info_outline wiederhergestellt. Auf Bahn 1 ein „CLUBHAUS“-Wegweiser-Label sauber neu gerendert, auf Bahn 9 ein zweites, unlesbares rotiertes Textelement auf Nutzerentscheidung wegretuschiert. Alle 9 Bahnenbilder nochmals einzeln geprüft, keine weiteren Funde.
- 20.09.2026: Korrektur zu Abschnitt 17 (Abschnitt 21): Nutzer meldete erneut asymmetrische Ränder im 9-Bahnen-Modus, diesmal in die andere Richtung (rechter Rand wirkte doppelt so breit wie der linke). Ursache war, dass der Abschnitt-17-Fix zwar rechnerisch symmetrisch war, die dabei rechts reservierte, aber leere Reiterleiste jedoch als große unbenutzte Fläche wahrgenommen wurde, während links dieselbe Breite durch sichtbare, gefüllte Reiter belegt war. Rechte Reiterleiste im 9-Bahnen-Modus wieder komplett aus dem Layout entfernt (Umkehrung von Abschnitt 17); auf simuliertem iPhone-Viewport verifiziert, `main_pager_flow_test.dart` angepasst.
