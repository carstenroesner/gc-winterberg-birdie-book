...
- **Namensfeld auf der Startseite** (Nutzerwunsch, 18.09.2026): Ein Eingabefeld für den eigenen Namen auf der Startseite ergänzen. Einmal eingegeben, soll der Name dauerhaft gespeichert und bei jedem künftigen App-Start automatisch vorbelegt/angezeigt werden (lokale Persistenz, analog zu Sprachauswahl/Schlägerliste).
- **9-Bahnen-Runde als Standardauswahl** (Nutzerwunsch, 18.09.2026): Bei „Neue Runde“ soll künftig die 9-Bahnen-Runde vorausgewählt sein statt wie bisher die 18-Bahnen-Runde (siehe Abschnitt 10). Betrifft den Default-Zustand des Umschalters in den Rundeneinstellungen.
- **Datum und Wetterbedingungen auf der Scorecard festhalten** (Nutzerwunsch, 18.09.2026): Das Datum wird auf der Scorecard bereits angezeigt (siehe Abschnitt 5e/Umsetzung), soll aber dauerhaft mit der gespeicherten Runde verknüpft werden. Zusätzlich sollen, wenn möglich, die aktuellen Wetterbedingungen zum Zeitpunkt der Runde erfasst und mit der Scorecard gespeichert werden, damit bei gespeicherten Runden später nachvollziehbar ist, bei welchem Wetter wie gespielt wurde (Datenquelle für Wetter noch zu klären, z. B. Wetter-API oder manuelle Eingabe).
- **Anderes Symbol statt Info-Icon für den Charakteristik-Text-Button** (Nutzerwunsch, 18.09.2026): Das aktuelle (i)-Info-Icon auf der Bahnseite (siehe Abschnitt 15) passt inhaltlich nicht gut, da es sich um einen Spieltipp und nicht um eine allgemeine Information handelt. Stattdessen ein Glühbirnen-/Idee-Symbol (oder ein anderes, für „Tipp“ passenderes Icon) verwenden.
- **Vollständige Übersetzung aller App-Texte auf Englisch und Niederländisch** (Nutzerwunsch, 18.09.2026): Bislang sind nur die UI-Textbausteine in `locale_service.dart` dreisprachig (DE/EN/NL), die Bahn-Charakteristik-Texte in `course_data.dart` liegen bewusst nur auf Deutsch vor (siehe Abschnitt 4a). Auf Wunsch sollen künftig auch diese Bahntexte (und alle sonstigen bislang nur deutschen Inhalte) ins Englische und Niederländische übersetzt und sprachabhängig angezeigt werden.
