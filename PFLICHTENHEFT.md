...
**Status: Umsetzung gestartet (14.09.2026) – erste Version 0.1 als PWA existiert, siehe Abschnitt 9. Am 15.09.2026 wurde die in Abschnitt 10 spezifizierte Erweiterung (Rundeneinstellungen, 9-/18-Loch-Modus, Wisch-Pager, individuelle Lochskizzen, „Problem melden") umgesetzt und nach GitHub gepusht. Am 16.09.2026 wurde mit der vollständigen Migration auf Flutter/Dart begonnen (neuer, für alle Apps verbindlicher App-Entwicklungsstandard – siehe Abschnitt 14); die bisherige Vanilla-JS-Version wird dadurch abgelöst.**
...
Vollständig umgesetzt: Projekt-Scaffold, `lib/theme/`, `lib/models/`, `lib/content/`, `lib/services/`, `main.dart`, `hole_sketch_painter.dart`/`hole_sketch.dart`, alle Pager-Widgets, alle ⋮-Menü-Ziele, `main_pager_screen.dart`, Start-/Rundenverlauf-/Sprach-Widgets, `clubs_screen.dart` mit Sub-Widgets, sowie eine Test-Suite (`widget_test.dart`, `hole_sketch_defs_test.dart`, `rounds_service_test.dart`, `clubs_service_test.dart`, `clubs_screen_test.dart`, `locale_service_test.dart`, `report_issue_dialog_test.dart`, `scorecard_page_test.dart`, `main_pager_flow_test.dart`) sowie der CI/CD-Workflow und `pubspec.yaml`. Noch offen: Commit, Push über die Geräte-Verbindung zum Mac (einziger Pfad mit GitHub-Zugriff in dieser Session), Abwarten des CI-Laufs, Live-Verifikation im Browser (siehe 14.6).

### 14.6 Noch zu erledigen

- GitHub-Pages-Deploy-Quelle in den Repo-Einstellungen von „Deploy from branch" auf „GitHub Actions" umstellen (siehe Punkt 15 in Abschnitt 6) – sonst liefert GitHub Pages weiterhin den alten, statischen Vanilla-JS-Stand aus.
- Push nach GitHub, CI-Lauf abwarten, bei Rot über Check-Run-Annotations diagnostizieren.
- Live-Version im Browser gegen die bisherige 19-Schritte-Prüfliste (siehe frühere Playwright-Smoke-Tests) plus die neuen Funktionsumfang-/Über-Prüfungen verifizieren.
